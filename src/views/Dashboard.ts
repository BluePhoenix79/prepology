...
export function renderDashboard(): HTMLElement {
  ...
  function renderGrid() {
    const questionsForSection = questionBank.filter(q => q.section === activeSection);
  ...
  function draw() {
    ...
    root.querySelector('#btn-practice-all')?.addEventListener('click', () => {
      store.startSession(activeSection, activeDifficulty);
    });
    ...
    root.querySelectorAll('.op-skill-row').forEach(row => {
      row.addEventListener('click', (e) => {
        ...
        store.startSession(activeSection, activeDifficulty, domain, skill);
      });
    });
  }
